...
<body>
    ...
    def _generate_toolbar(self):
        """Genera toolbar con pulsanti Stampa e Salva PDF"""
        return """
    ...
    def _generate_javascript(self):
        """Genera JavaScript per PDF via window.print() (Chrome-native, qualità perfetta)"""
        return """
    <script>
        /**
         * savePDF - Soluzione brillante:
         * Usa window.print() di Chrome che produce PDF perfetto con @media print CSS.
         * Zero dipendenze server. Qualità identica all'HTML.
         *
         * Fallback: Puppeteer via server localhost:7860 se disponibile.
         */
        async function savePDF() {
            const toolbar = document.getElementById('mainToolbar');

            // Strategia 1: print nativo del browser (Chrome/Edge → Salva come PDF)
            // È la qualità più alta possibile - usa lo stesso engine di rendering
            if (window.chrome || navigator.userAgent.includes('Chrome') || navigator.userAgent.includes('Edg')) {
                // Nascondi toolbar per stampa pulita
                toolbar.style.display = 'none';

                // Imposta il titolo per il nome file PDF suggerito
                const origTitle = document.title;
                document.title = 'SUV_QC_Report_' + new Date().toISOString().split('T')[0];

                // Listener per ripristinare toolbar dopo stampa
                const restore = () => {
                ...
                // Apri dialog di stampa → l'utente sceglie "Salva come PDF"
                // Chrome pre-seleziona "Salva come PDF" come destinazione
                window.print();
                return;
            }

            // Strategia 2: Puppeteer via server (per Firefox/Safari o uso headless)
            ...
            try {
                ...
                toolbar.style.display = '';
                // Fallback finale: apri print dialog comunque
                console.warn('Puppeteer non disponibile, uso print nativo:', error.message);
                toolbar.style.display = 'none';
        ...
        // Scorciatoia da tastiera Ctrl+S → Salva PDF
